...
public class Solution {
    public static void main(String[] args) throws IOException {
        ...
        int openTagIndex = strBuilder.indexOf(openTag);

        while (openTagIndex != -1)
        {
            tagIndexes.put(openTagIndex, true);
            openTagIndex = strBuilder.indexOf(openTag, openTagIndex + 1);
        }

        int closeTagIndex = strBuilder.indexOf(closeTag);
        while (closeTagIndex != -1)
        {
            tagIndexes.put(closeTagIndex, false);
            closeTagIndex = strBuilder.indexOf(closeTag, closeTagIndex + 1);
        }
        ...
        for (Map.Entry<Integer, Integer> entry : finalIndexes.entrySet())
            System.out.println(strBuilder.substring(entry.getKey(), entry.getValue() + closeTag.length()));
    }
}
